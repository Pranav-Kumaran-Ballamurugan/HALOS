...
class HALOSApp:
    def __init__(self, root):
        ...
        self.chat_history = []  # context-aware memory
    ...
    def handle_command(self, command):
        ...
        if len(self.chat_history) > 10:
            self.chat_history = self.chat_history[-10:]  # keep only recent history

        try:
            response = openai.ChatCompletion.create(
                model=os.getenv("GPT_MODEL", "gpt-4-turbo"),
                messages=[{"role": "system", "content": "You're a helpful assistant."}] + self.chat_history
            )
