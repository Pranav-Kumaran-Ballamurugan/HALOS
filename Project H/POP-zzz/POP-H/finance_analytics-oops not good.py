#!/usr/bin/env python3
# halos_main.py - FULL CODE (no scaffold)
...
class HALOSApp(tk.Tk):
    ...
    def setup_payment_tab(self):
        ...
        self.amount_entry = ttk.Entry(frame); self.amount_entry.insert(0, "49.99"); self.amount_entry.pack(side=tk.LEFT, padx=5)
        self.email_entry = ttk.Entry(frame); self.email_entry.insert(0, "customer@example.com"); self.email_entry.pack(side=tk.LEFT, padx=5)
    ...
    def process_payment(self):
        ...
        txn = {"id": txn_id, "date": datetime.now().isoformat(), "amount": float(amount), "currency": "USD", "method": method.value, "status": "initiated", "user_email": email, "metadata": {}}
        ok, msg = self.core.transaction_manager.add_transaction(txn)
        if not ok: messagebox.showerror("Error", msg); return
        self.status = ttk.Label(self, text="Processing payment..."); self.status.pack()
        threading.Thread(target=self._process_payment_bg, args=(method, amount, email, txn_id), daemon=True).start()
    ...
    def load_transactions(self):
        for i in self.transaction_tree.get_children(): self.transaction_tree.delete(i)
